...
def main(
    ...
    transcript = transcript.loc[
        transcript.iloc[:, GTF_TYPE_IX] != "transcript"
    ]
    cds = pd.read_csv(
        f"{cds_only_path}",
        sep="\t",
    ...
    cds["width"] = (
        cds.iloc[:, GTF_END_IX]
        - cds.iloc[:, GTF_START_IX]
        + ZERO_ONE_INDEX_OFFSET
    )
    transcript["cumwidth"] = transcript.groupby("transcript_id")[
        "width"
    ].transform(lambda x: pd.Series(x).cumsum().shift(fill_value=0.0) + 1)
    ...
    transcript["cumwidth"] = transcript.groupby("transcript_id")[
        "width"
    ].transform(lambda x: pd.Series(x).cumsum())
